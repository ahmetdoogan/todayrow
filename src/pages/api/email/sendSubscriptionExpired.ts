...
<div style="background-color: #f8fafc; padding: 40px 0;">
  <div style="max-width: 600px; margin: 0 auto; background-color: #ffffff; border-radius: 16px; padding: 40px; box-shadow: 0 4px 6px rgba(0,0,0,0.05);">
    ...
    <h1 style="color: #1e293b; font-size: 24px; font-weight: 600; text-align: center; margin: 0 0 16px;">
      Your Todayrow Pro Subscription Has Expired
    </h1>
    ...
    <div style="background-color: #f1f5f9; border-radius: 12px; padding: 24px; margin-bottom: 32px; text-align: center;">
      <p style="color: #64748b; font-size: 16px; line-height: 24px; margin: 0 0 16px;">
        We were unable to process your subscription renewal. Please check your payment method and reactivate your Pro subscription.
      </p>
      <p style="color: #64748b; font-size: 16px; line-height: 24px; margin: 0;">
        Continue enjoying all premium features by renewing your Pro subscription today.
      </p>
    </div>
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    res.status(200).json({ message: "Subscription expired email sent successfully" });
  } catch (error) {
    console.error("Error sending subscription expired email:", error);
    res.status(500).json({ message: "Error sending subscription expired email" });
  }
}