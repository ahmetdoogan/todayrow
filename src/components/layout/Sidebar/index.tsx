...
export default function Sidebar({
  ...
  const handleSearchChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    searchTimeoutRef.current = setTimeout(() => {
      ...
      contents.forEach((content) => {
        ...
        if (isExactTitleMatch) exactTitleMatches.push({ ...content, type: "content" });
        else if (isTitleMatch || isContentMatch) contentMatches.push({ ...content, type: "content" });
      });
      ...
      notes.forEach((note) => {
        ...
        if (isExactTitleMatch) exactTitleMatches.push({ ...note, type: "note", details: note.content });
        else if (isTitleMatch || isContentMatch) contentMatches.push({ ...note, type: "note", details: note.content });
      });
      ...
      plans.forEach((plan) => {
        ...
        if (isExactTitleMatch) exactTitleMatches.push({ ...plan, type: "plan" });
        else if (isTitleMatch || isDetailsMatch) contentMatches.push({ ...plan, type: "plan" });
      });
  ...
  // RENDER
  if (!hydrated || subLoading) return null;
    ...
    <>
      ...
      >
        <div className="flex flex-col h-full relative p-3">
          ...
          >
            <ChevronLeft
              className={`w-4 h-4 text-gray-500 dark:text-gray-400 transition-transform duration-300 ${isCollapsed ? "rotate-180" : ""}`}
            />
          </button>
                      ...
                      <div className="flex items-start">
                        <div className="flex-1 min-w-0">
                          <div className="font-medium text-gray-800 dark:text-gray-200 line-clamp-1 flex justify-between items-center gap-2">
                            ...
                            <span className={`text-xs px-2 py-0.5 rounded-full ${result.type === "content" ? "bg-blue-500/20 text-blue-500" : result.type === "note" ? "bg-green-500/20 text-green-500" : "bg-violet-500/20 text-violet-500"}`}>
              ...
              {navItems.map((item) => {
                ...
                return (
                  <Link
                    ...
                    className={`flex items-center gap-2 px-3 py-2 rounded-lg text-xs text-gray-600 dark:text-gray-300 hover:text-gray-800 dark:hover:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700 ${isActive ? "bg-gray-200 dark:bg-gray-700 text-gray-800 dark:text-gray-200" : ""}`}
                  >
